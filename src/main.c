...
int main(void){
    ...
    uart_init(115200);
    difficulty=2;
    while(difficulty != 0){ // exit when quit is pressed. When game is over, return here
        ...
        while( difficulty != 0 && playerHits < 3 ){ // skip this part if player wants to quit
            /*
            Game Start
            */
            ...
            setTimer();
            initCounter(&counter);
            // speed set by difficulty, player speed unchanged
            ...
            while(playerHits<3 && difficulty !=0){ // main game loop, while player is not dead
                ...
                */
                ...
                //Spawn enemies in intervals, ignore enemies which are still alive
